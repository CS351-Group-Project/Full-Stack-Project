...
export default function Profile({ authUser }) {
  const [user, setUser] = useState(null); // this will hold the profile object
  const [uploading, setUploading] = useState(false);
  ...
  useEffect(() => {
    const load = async () => {
      if (!authUser || !authUser.profile) return;

      try {
        // Use the profile id returned by login/register
  ...
  const handleFileChange = async (e) => {
    ...
    try {
      setUploading(true);
      // user.user_id comes from serializer and is the Django auth user id
      const data = await apiUploadPicture(file, user.user_id);
      const profile = data.user;
  ...
  return (
    <div className="page">
      ...
      {!authUser ? (
        ...
        <>
          {/* Avatar + editable form (your existing JSX, using new handlers) */}
          <div
            style={{
              display: "flex",
              gap: "1.5rem",
          ...
          {/* keep the rest of your “About CultureConnect” section as is */}
        </>
      )}
    </div>
  );
